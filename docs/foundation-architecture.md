...
这里的“每个系统都支持”指 Windows、macOS、Linux 都能分别安装和本地运行，不是跨设备同步。Windows 是第一个实现和验证平台，不是核心层的永久边界。
...
首个基础版本不做：跨设备同步、云端账号、团队共享、macOS/Linux 同期实现、物理移动文件、管理员常驻服务、内核驱动、全盘默认采集、屏幕截图、键盘记录、自动删除、自动任务切换、插件市场、微服务或远程数据库。macOS/Linux 是后续平台适配阶段，不是被排除的产品目标。
